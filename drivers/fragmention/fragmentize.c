...
static unsigned long accupy = INIT_VALUE_ZERO;
static unsigned long real_accupy = INIT_VALUE_ZERO;
static unsigned long *pfn_occupy = INIT_VALUE_NULL;
static int filled = INIT_VALUE_ZERO;

static struct kobject *fragmentize_kobj;

static ssize_t alloc_times_total_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (0 == block_order)
		return snprintf(buf, PAGE_SIZES, "%u\n", filled);
	else
		return snprintf(buf, PAGE_SIZES, "%u\n", filled / ((1 << block_order) / 2));
}

static ssize_t alloc_size_total_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZES, "%u MBytes\n", (filled << PAGE_SHIFT) / SZ_1M);
}

static DEVICE_ATTR(alloc_times_total, 0444, alloc_times_total_show, NULL);

static DEVICE_ATTR(alloc_size_total, 0444, alloc_size_total_show, NULL);
...
static int __init fragmentize_init(void)
{
	...
	if (ret < 0) {
		...
		pr_err("Error creating sysfs group");
		return -ENOMEM;
	}
