	...
	struct goodix_ts_board_data *board_data)
{
	...
	if (r < 0) {
		...
		pr_debug("Parse reset-gpio[%d] from dt", r);
	...
	if (r < 0) {
		...
		pr_debug("Parse irq-gpio[%d] from dt", r);
	...
	unsigned char *data, unsigned int len)
{
	...
	while (pos != len) {
		...
		for (retry = 0; retry < GOODIX_BUS_RETRY_TIMES; retry++) {
			...
			pr_debug("I2c read retry[%d]:0x%x", retry + 1, reg);
			msleep(20);
		}
		...
		unsigned char *data, unsigned int len)
{
	...
	while (pos != len) {
		...
		for (retry = 0; retry < GOODIX_BUS_RETRY_TIMES; retry++) {
			...
			pr_debug("I2c write retry[%d]", retry + 1);
			msleep(20);
		}
		if (unlikely(retry == GOODIX_BUS_RETRY_TIMES)) {
...
int goodix_set_i2c_doze_mode(struct goodix_ts_device *dev, int enable)
{
	...
	if (enable) {
		...
		if (dev->doze_mode_set_count == 0) {
			...
			/*pr_debug("doze count not euqal 0, so skip doze mode enable");*/
			result = 0;
			goto exit;
		}
	} else {
...
#define IS_CHAR(c)	(((c) >= 'A' && (c) <= 'Z')\
	...
	if (sensor_id_mask != 0) {
		...
		pr_debug("sensor_id_mask:0x%02x, sensor_id:0x%02x",
	...
	pr_debug("PID:%s,SensorID:%d, VID:%*ph",
						version->pid,
						version->sensor_id,
...
#define IS_CHAR(c)	(((c) >= 'A' && (c) <= 'Z')\
	...
	if (checksum_u8(buffer, sizeof(buffer)) == 0 &&
		...
		if (version) {
			...
			if (version->cid)
				pr_debug("PID:%s,CID: %c,VID:%04x,SensorID:%u",
						version->pid, version->cid + 'A' - 1,
						version->vid, version->sensor_id);
			else
				pr_debug("PID:%s,VID:%04x,SensorID:%u",
		...
		struct goodix_ts_config *config)
{
	...
	r = 0;
	pr_debug("send small cfg SUCCESS");
		...
		struct goodix_ts_config *config)
{
	...
	pr_debug("Send large cfg SUCCESS");
	r = 0;
...
static int goodix_check_cfg_valid(struct goodix_ts_device *dev, u8 *cfg, u32 length)
{
	...
	pr_debug("cfg bag_num:%d, cfg length:%d", bag_num, length);
	...
	ret = 0;
	pr_debug("configuration check SUCCESS");
		...
		struct goodix_ts_config *config)
{
	...
	pr_debug("Send %s,ver:%02xh,size:%d",
...
static int goodix_close_hidi2c_mode(struct goodix_ts_device *ts_dev)
{
	...
	if (try_times >= 10) {
		pr_debug("goodix_close_hidi2c_mode FAILED, 0x8040 is not equal to 0xff");
		return -EINVAL;
	}
		...
		if (ts_cmd.initialized) {
			...
			for (j = 0; j < 3; j++) {
				if (goodix_i2c_read(ts_dev, 0x8040, buffer, 1) != 0)
					...
					if (buffer[0] != 0xFF) {
						pr_debug("try_times:%d:%d, read 0x8040:0x%02x",
...
exit:
	if (try_times >= 3) {
		pr_debug("close hid_i2c mode FAILED");
		r = -EINVAL;
	} else {
		pr_debug("close hid_i2c mode SUCCESS");
		r = 0;
	}
	return r;
}
										  ...
										  u8 *buf)
{
	...
	pr_debug("config_version:%u, vub_bags:%u",
			buf[0], sub_bags);
	for (i = 0; i < sub_bags; i++) {
		...
		pr_debug("sub bag num:%u,sub bag length:%u", buf[offset], subbag_len);
					      ...
					      u8 *config_data, u32 config_len)
{
	...
	if (r > 0)
		pr_debug("success read config, len:%d", r);
	/* clear command */
...
static int goodix_hw_init(struct goodix_ts_device *ts_dev)
{
	...
	if (ts_dev->ic_type == IC_TYPE_NANJING) {
		...
		if (r < 0)
			pr_debug("close hid i2c mode FAILED");
	}
...
int goodix_hw_reset(struct goodix_ts_device *dev)
{
	...
	if (dev->reg.esd) {
		...
		pr_debug("reg.esd is NULL, skip dynamic esd init");
		...
		struct goodix_request_data *request_data)
{
	...
	switch (buffer[0]) {
	case REQUEST_CONFIG:
		pr_debug("HW request config");
		goodix_send_config(dev, dev->normal_cfg);
		goto clear_requ;
		break;
	case REQUEST_BAKREF:
		pr_debug("HW request bakref");
		goto clear_requ;
		break;
	case REQUEST_RESET:
		pr_debug("HW requset reset");
		goto clear_requ;
		break;
	case REQUEST_MAINCLK:
		pr_debug("HW request mainclk");
		goto clear_requ;
		break;
	default:
		pr_debug("Unknown hw request:%d", buffer[0]);
		return 0;
	}
		...
		u8 *coor_buf, u32 coor_buf_len, int touch_num)
{
	...
	for (i = 0; i < sizeof(remap_array); i++) {
		if (remap_array[i] == 0xff)
			...
			if (need_leave == true) {
				/*pr_debug("---leave, trace id:%d:%d", remap_array[i], i);*/
				remap_array[i] = 0xff;
			}
		}
	}
		...
		if (coor_buf[offset] >= 0x80) {
			...
			for (j = 0; j < sizeof(remap_array); j++) {
				if (remap_array[j] == coor_buf[offset]) {
					/*pr_debug("***remap, %d--->%d", coor_buf[offset], j);*/
					coor_buf[offset] = j;
					break;
				}
			}
	...
	/*for (i = 0; i < touch_num; i++) {
		pr_debug("remap data%d:0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
	...
	/*for (i = 0; i < touch_num; i++) {
		pr_debug("final data%d:0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
				i, coor_buf[i * 8], coor_buf[i * 8 + 1],
				coor_buf[i * 8 + 2], coor_buf[i * 8 + 3],
				coor_buf[i * 8 + 4], coor_buf[i * 8 + 5],
				coor_buf[i * 8 + 6], coor_buf[i * 8 + 7]);
		...
		u8 *pre_buf, u32 pre_buf_len)
{
	...
	/*pr_debug("$$$$$$coord_sta:0x%02x, have_key:%d, key_value:0x%02x",
			coord_sta, touch_data->have_key, touch_data->key_value);*/

	/*for (i = 0; i < touch_num; i++) {
		pr_debug("raw coor data%d:0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
		...
		struct goodix_ts_event *ts_event)
{
	...
	if (likely((event_sta & GOODIX_TOUCH_EVENT) == GOODIX_TOUCH_EVENT)) {
		...
		pr_debug("Gesture event");
	} else if ((event_sta & GOODIX_HOTKNOT_EVENT) == GOODIX_HOTKNOT_EVENT) {
		/* handle hotknot event */
		pr_debug("Hotknot event");
	} else {
		pr_debug("unknow event type:%02x", event_sta);
		r = -EINVAL;
	}
...
static int goodix_esd_check(struct goodix_ts_device *dev)
{
	...
	if (r < 0 || (data == GOODIX_ESD_TICK_WRITE_DATA)) {
		pr_debug("dynamic esd occur, r:%d, data:0x%02x", r, data);
		r = -EINVAL;
		goto exit;
	}
	...
	if (dev->ic_type == IC_TYPE_NANJING) {
		...
		if (r < 0 || (data != 0xaa)) {
			pr_debug("static esd occur, r:%d, data:0x%02x", r, data);
			r = -EINVAL;
			goto exit;
		}
	}
